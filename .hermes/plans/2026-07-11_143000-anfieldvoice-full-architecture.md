...
| **NFC engine** | `react-native-nfc-hce` + `expo-nfc` | Phone-as-tag gate access (Android HCE) |
...
#### 3.7 nfc_credentials (Phone-as-Tag)
...
CREATE TABLE nfc_credentials (
    ...
    user_id         INT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    apartment_id    INT NOT NULL REFERENCES apartments(apartment_id),
    -- Physical tag: the UID printed on the fob/tag
    tag_uid         VARCHAR(50) UNIQUE,
    -- Phone credential: generated by the app when phone NFC is activated
    phone_token     VARCHAR(100) UNIQUE,
    credential_type VARCHAR(10) NOT NULL CHECK (credential_type IN ('tag', 'phone')),
    -- Mutual exclusivity: when phone is active, tag is disabled and vice versa
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
...
COMMENT ON TABLE nfc_credentials IS 'NFC access credentials for gate entry. '
    'tag = physical fob/card. phone = phone emulating tag via HCE. '
    'Mutually exclusive per apartment: activating phone deactivates the physical tag.';

CREATE INDEX idx_nfc_tag_uid ON nfc_credentials (tag_uid) WHERE is_active = TRUE;
CREATE INDEX idx_nfc_phone_token ON nfc_credentials (phone_token) WHERE is_active = TRUE;
CREATE INDEX idx_nfc_user_apartment ON nfc_credentials (user_id, apartment_id);

-- Gate access log (every tap recorded)
...
CREATE INDEX idx_gate_access_apartment ON gate_access_log (apartment_id, created_at DESC);
CREATE INDEX idx_gate_access_credential ON gate_access_log (credential_id);
```
...
#### 4.7 NFC Credentials (Phone-as-Tag)

| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/api/v1/nfc/activate-phone` | Activate phone NFC for an apartment (deactivates physical tag) |
| `POST` | `/api/v1/nfc/deactivate-phone` | Deactivate phone NFC (reactivates physical tag) |
| `POST` | `/api/v1/nfc/verify` | Gate reader verifies a credential (tag UID or phone token) |
| `GET` | `/api/v1/nfc/credentials` | List user's NFC credentials and their status |
| `POST` | `/api/v1/nfc/register-tag` | Admin registers a physical tag UID for a resident |
| `DELETE` | `/api/v1/nfc/credentials/{id}` | Remove a credential (tag or phone) |
| `GET` | `/api/v1/nfc/access-log` | Gate access history for an apartment |
...
└── Tab Navigator ((tabs)/_layout.tsx)
    ...
    │
    ├── 📱 NFC Access (nfc.tsx) ← NEW
    │   ├── Phone NFC toggle (activate/deactivate)
    │   ├── Shows credential status (tag active / phone active)
    │   ├── Access history log
    │   └── iOS: QR code fallback banner when NFC unavailable
    │
...
#### 9.5 NFC Phone-as-Tag Platform Constraints

| Risk | Source | Impact | Mitigation |
|------|--------|--------|------------|
| **iOS has NO Host Card Emulation** | Apple Core NFC docs | Critical — iPhones cannot act as NFC tags | iOS fallback: QR code at gate reader. Add a "Scan QR" banner to the NFC screen. QR encodes the same phone_token. |
| **Android HCE requires API 19+** | Android docs | Minor — most devices support it | Target API 24+ (99.99% of active Android devices). Detect at runtime and show fallback. |
| **HCE needs a service declaration in AndroidManifest** | Expo plugin limitation | Requires an Expo config plugin | Build a custom `expo-nfc-hce` plugin or eject for this feature. Can't do it with bare config. |
| **NFC antenna placement varies** | Hardware | UX — user taps wrong spot | Show phone diagram with NFC antenna location. Detect failed reads and retry. |
| **Gate reader must poll API, not just check raw UID** | Architecture | Reader firmware may need update | Gate reader sends tag_uid or phone_token to `POST /api/v1/nfc/verify`. Backend checks is_active, mutual exclusivity, and apartment permissions. Returns grant/deny. |
| **Mutual exclusivity race condition** | Concurrency | Both tag and phone could be briefly active | Use database-level locking (`SELECT ... FOR UPDATE`) when toggling credential state. |
| **Phone battery death** | Operational | Resident locked out | Physical tag still works if phone NFC is deactivated. Encourage keeping the tag as backup. |
| **Reader <-> backend latency** | Network | Slow gate opens | Cache active credentials locally on the reader (Redis-backed whitelist with short TTL). Push credential changes to the reader via WebSocket. |
...
|Slice 8:  NFC phone-as-tag (Android HCE + iOS QR fallback)
|Slice 9:  Remaining screens (directory, residents tab, history)
|Slice 10: Store submission prep + final polish
```
...
| D11 | **NFC phone-as-tag via Android HCE + iOS QR fallback** | Android supports Host Card Emulation natively. iOS does not — QR code at reader is the only platform-agnostic alternative. Mutual exclusivity enforced at DB level. | July 2026 |
| D12 | **Gate reader polls API for credential verification** | Reader sends tag_uid or phone_token to `/api/v1/nfc/verify`. Backend checks is_active, permissions, and mutual exclusivity. Reader caches active credentials with short TTL. | July 2026 |
